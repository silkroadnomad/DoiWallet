...
describe('multisig-wallet (p2sh)', () => {
  ...
  it.skip("can coordinate tx creation", async () => {
    ...
    assert.strictEqual(w.getCosignerForFingerprint(fp1cobo), w.getCosigner(1));
    assert.strictEqual(
      w.getCosignerForFingerprint(fp2coldcard),
      w.getCosigner(2)
    );
    assert.strictEqual(w.getFingerprint(1), fp1cobo);
    ...
    assert.strictEqual(
      ...
      "3J5xQcgBqoykSHhmDJLYp87SgVSNhYrvnz"
    );
    assert.strictEqual(
      w._getExternalAddressByIndex(1),
      "3GkEXFYUifSmQ9SgzJDWL37pjMj4LT6vbq"
    );
    assert.strictEqual(
      w._getInternalAddressByIndex(0),
      "365MagKko4t7L9XPXSYGkFj23dknTCx4UW"
    );
    assert.strictEqual(
      w._getInternalAddressByIndex(1),
      "36j8Qx6vxUknTxGFa5yYuxifEK9PGPEKso"
    );
    assert.ok(!w.isWrappedSegwit());
    assert.ok(!w.isNativeSegwit());
    assert.ok(w.isLegacy());
    ...
    assert.ok(
      !tx,
      "tx should not be provided when PSBT is only partially signed"
    );
  ...
  it.skip("can do both signatures", () => {
    ...
    assert.strictEqual(
      w._getInternalAddressByIndex(3),
      "3PmqRLiPnBXhdYGN6mAHChXLPvw8wb3Yt8"
    );
    assert.ok(!w.isWrappedSegwit());
    assert.ok(!w.isNativeSegwit());
    assert.ok(w.isLegacy());
...
describe('multisig-wallet (native segwit)', () => {
  ...
  it.skip('can sort buffers', async () => {
    let sorted;
  ...
  it.skip('can export/import when one of cosigners is mnemonic seed', async () => {
    const path = "m/48'/0'/0'/2'";
  ...
  it.skip('can import txt from Cobo and export it back', async () => {
    const path = "m/48'/0'/0'/2'";
  ...
  it.skip('can parse p2sh colcard format from caravan', () => {
    const cc =
  ...
  it.skip('can import from specter-desktop/fullynoded (p2sh-p2wsh)', () => {
    // @see https://github.com/Fonta1n3/FullyNoded/blob/master/Docs/Wallets/Wallet-Export-Spec.md
  ...
  it.skip('can generate proper addresses for wallets with passphrases. Export and import such wallet', () => {
